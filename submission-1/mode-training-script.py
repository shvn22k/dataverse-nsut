...
xgb_random = RandomizedSearchCV(
    xgb.XGBClassifier(random_state=42, tree_method=xgb_tree_method, eval_metric='logloss'),
    xgb_param_grid,
    n_iter=15,
    scoring='roc_auc',
    cv=3,
...
try:
    lgb_random = RandomizedSearchCV(
        lgb.LGBMClassifier(random_state=42, verbose=-1, device=lgb_device),
        lgb_param_grid,
        n_iter=15,
        scoring='roc_auc',
        cv=3,
    ...
    if USE_GPU:
        ...
        lgb_device = 'cpu'
        lgb_random = RandomizedSearchCV(
            lgb.LGBMClassifier(random_state=42, verbose=-1, device='cpu', force_col_wise=True),
            lgb_param_grid,
            n_iter=15,
            scoring='roc_auc',
            cv=3,
...
print("XGBoost...")
t0 = time.time()
xgb_model = xgb.XGBClassifier(**xgb_best_params, random_state=42, tree_method=xgb_tree_method, 
...
submission = pd.DataFrame({
    id_col: test_ids if test_ids is not None else range(len(test_pred_ensemble)),
